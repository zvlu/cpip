...
export async function POST(req: NextRequest) {
  ...
  for (const c of cc) {
    const { data: posts } = await supabase.from("posts").select("*, revenue_estimates(*)").eq("creator_id", c.creator_id).eq("campaign_id", campaign_id).gte("posted_at", thirtyDaysAgo).order("posted_at", { ascending: false });
    const { data: prev } = await supabase.from("performance_scores").select("*").eq("creator_id", c.creator_id).eq("campaign_id", campaign_id).order("score_date", { ascending: false }).limit(1).single();
