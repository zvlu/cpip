...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const records = posts.map((p: any) => ({
      creator_id, tiktok_post_id: p.id, url: p.url, caption: p.caption,
      hashtags: p.hashtags, views: p.views, likes: p.likes, comments: p.comments,
      shares: p.shares, saves: p.saves, duration_seconds: p.duration,
      posted_at: p.posted_at, has_product_link: p.has_product_link, scraped_at: new Date().toISOString(),
    }));
