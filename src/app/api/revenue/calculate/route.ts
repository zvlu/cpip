...
export async function POST(req: NextRequest) {
  ...
  const estimates = posts.map((post) => ({
    post_id: post.id, campaign_id, views: post.views,
    ctr: campaign.default_ctr, cvr: campaign.default_cvr,
    aov: campaign.aov, commission_rate: campaign.commission_rate,
  }));
